...
简历选择器旁提供 `↶` 撤销和 `↷` 重做。文字输入、内容新增/删除/移动和排版调整使用同一条编辑历史。
...
- [V2.5 本机草稿恢复实施计划](docs/plans/2026-07-12-v2-5-local-draft-recovery.md)
- [V2.4 当前边界](docs/v2-4-boundaries.md)
- [V2.4 验收清单](docs/v2-4-acceptance-checklist.md)
- [V2.4.2 正文预览缓冲设计](docs/plans/2026-07-11-v2-4-2-content-preview-buffering-design.md)
...
- ZIP 不包含可重新生成的 `output/`、浏览器中的未保存草稿、本机 `.env.local` 或迁移状态文件。
...
调整字号、行高、内容间距或页边距时，编辑器会复用当前预览文档，并在不可见的 A4 克隆上测试自动适配候选；可见简历只应用最终候选一次，滑块也不会在拖动时被重建。修改姓名、经历、技能或项目等内容时，新正文同样会先在当前 iframe 的不可见页面中完成测量，再连同最终排版参数一次性替换可见 A4；已有预览不会重新加载 iframe。首次打开尚无可用页面的简历时仍会执行一次正常 HTML 加载。

存在未保存草稿时，刷新、关闭页面或离开编辑器会触发浏览器确认提示。保存成功后不再提示。

编辑过程中，未保存内容还会按简历 ID 暂存在当前浏览器的本地存储中。意外刷新、浏览器崩溃或电脑重启后，再次打开同一份简历会提供 `暂不处理`、`放弃草稿` 和 `恢复草稿`：

- `恢复草稿` 只恢复到编辑表单并保持“未保存”状态，仍需手动点击 `保存` 才会写入 YAML。
- `暂不处理` 保留正式 YAML 和本机草稿，方便下次再决定。
- `放弃草稿` 或保存成功后会删除该份简历的本机草稿。
- 草稿只存在于当前浏览器，不跨电脑同步，也不进入数据导出 ZIP；清理该网站的浏览器数据会同时删除草稿。
...
点击左侧工具栏中的 `↶` 和 `↷`。也可以使用 `Cmd/Ctrl + Z` 撤销、`Cmd/Ctrl + Shift + Z` 重做；Windows/Linux 还支持 `Ctrl + Y`。同一字段连续输入会合并为一步，新增、删除、移动和排版调整各算一步。保存不会立即清空历史，因此保存后仍可撤销，但撤销后的内容会重新标记为未保存。

编辑历史最多保留当前会话中的 50 步，切换简历、载入样例、替换照片、恢复备份或刷新页面后重置。V2.5 本机草稿可以在刷新后恢复内容，但不会恢复撤销链。
...
重新启动 `npm run editor`，并在同一个浏览器中打开编辑器。进入对应简历时会显示“发现未保存草稿”，选择 `恢复草稿` 后核对内容，再手动保存。如果正式 YAML 在草稿产生后已经变化，弹窗会额外提示先核对差异。
...
- 浏览器本机草稿只用于异常恢复，不等于正式保存，也不提供跨设备同步。
- 编辑历史栈只保留当前页面会话，不进入 YAML 或数据包；撤销/重做后的当前内容仍按 V2.5 规则写入或清理浏览器本机草稿。
